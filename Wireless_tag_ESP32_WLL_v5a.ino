...
#include <WiFi.h>
#include <Wire.h>
#define USER_SETUP_INFO "WT32SC01Setup.h"
...
// #define GLCD 0 if you want to use GLCD fonts using set font (0)
//#define FS97 &FreeSans9pt7c
#define degcode 247
...
void setup() {
  ...
  //tft.setFreeFont(FS97);
  tft.setTextFont(4);  // 1 is the default, 2 is OK but lacks extended chars. 3 is the same as 1, but 4 is OK
...
void loop() {
  if (ts.touched()) {
    ...
    //Serial.printf("%d %d\n", p.x, p.y);  // for position diagnostics
    if (p.x < 290 && p.y < 50) {         // If upper top left touched then signal rescan of weather data
      Serial.println("Refreshing data...");
      clear_screen();
      Refresh = true;
      DrawButton(240, 0, 50, 50, 8, TFT_RED, TFT_BLACK, "R");
    }
  }
...
void DrawButton(int x, int y, int width, int height, int round, int buttonColour, int buttonTextColour, String buttonText) {
  tft.fillRoundRect(x, y, width, height, round, buttonColour);
  tft.setTextColor(buttonTextColour);
  tft.setCursor(x + width / 2 - 10, y + height / 2 - 10);
